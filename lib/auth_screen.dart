...
class _AuthScreenState extends State<AuthScreen>
  ...
  // ✅ FIXED: Use named route to load MainNavigationWrapper + BottomNav
  Future<void> _skipLogin() async {
    Navigator.pushReplacementNamed(context, '/home');
  }

  // ── LOGIN — only registered emails work ──
  Future<void> _handleLogin() async {
    ...
    if (result['success'] == true) {
      ...
      // Show exact error from backend (invalid email or password)
      setState(() => _errorMessage = result['message']);
    }
  }

  // ── REGISTER — auto login and go to home page after signup ──
  Future<void> _handleRegister() async {
    ...
    if (result['success'] == true) {
      ...
      if (loginResult['success'] == true) {
        ...
        // Registration worked but auto-login failed — go to login screen
        setState(() {
          _successMessage = 'Account created! Please sign in.';
          _isLoading = false;
        });
        await Future.delayed(const Duration(seconds: 2));
        if (mounted) _switchMode();
      }
    } else {
      setState(() {
            ...
            children: [
              ...
              if (_isLoading)
                Container(
                  ...
                  child: const Center(
                    child:
                        CircularProgressIndicator(color: Color(0xFF366000)),
                  ),
                ),
            ],
          ),
  ...
  Widget _buildUserTypeDropdown() {
    return Container(
      ...
      child: DropdownButtonFormField<String>(
        ...
        hint: Text('User Type',
            style:
                TextStyle(color: deepGreen.withOpacity(0.4), fontSize: 14)),
        items: const [
          DropdownMenuItem(value: 'Farmer', child: Text('Farmer')),
        ],
        onChanged:
            _isLoading ? null : (v) => setState(() => _selectedUserType = v),
    ...
    return Row(
      ...
      children: [
        Text(
          ...
          style:
              TextStyle(color: deepGreen.withOpacity(0.6), fontSize: 14),
        ),
        MouseRegion(
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        enabledBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(14),
            borderSide:
                BorderSide(color: lightGreen.withOpacity(0.3), width: 1)),
        focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(14),
            borderSide:
                const BorderSide(color: Color(0xFF366000), width: 1.5)),
  ...
  Widget _buildPasswordField({
    ...
    return TextFormField(
      ...
      validator: validator ??
          (v) => v!.isEmpty ? 'Please enter your password' : null,
    );
  }
}