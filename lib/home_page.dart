...
class _HomePageState extends State<HomePage> {
  ...
  // ── ✅ CORE FEATURE HANDLER WITH ACCESS CONTROL ──
  void _handleFeature(String title, {bool requiresLogin = false}) {
    // 🌐 Always free - opens website
    if (title == 'Visit Our Site') {
      _launchWebsite();
      return;
    }

    // 🌤️ Weather - FREE for all
    if (title == 'Weather') {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const WeatherScreen()),
    ...
    if (requiresLogin && !isLoggedIn) {
      ...
      // Optional: Auto-navigate to login after short delay
    ...
    // ✅ Allowed features with login satisfied
    if (title == 'Soil Scanner') {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const SoilScannerScreen()),
  ...
  Future<void> _launchWebsite() async {
    final Uri url = Uri.parse('https://yuccaconsult.com');
  ...
  // ── ✅ Feature Grid with Correct Access Rules ──
  Widget _buildFeatureGrid() {
    final features = [
  ...
  Widget _featureItem(IconData icon, String title,
    ...
    return Material(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(18),
        onTap: requiresLogin && !isLoggedIn
            ? () => _handleFeature(title, requiresLogin: true)
            : () => _handleFeature(title, requiresLogin: requiresLogin),
        child: Container(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(18),
            border: Border.all(color: color.withOpacity(0.2), width: 1.5),
          ...
          child: Column(
            ...
            children: [
              ...
              // 🔐 Show lock badge for premium features
              if (requiresLogin)
                Padding(
                  padding: const EdgeInsets.only(top: 4),
                  child: Row(
  ...
  // ── Main CTA: Visit Our Site (Always Free) ──
  Widget _buildMainCTA() {
    return SizedBox(
      height: 52,
      child: ElevatedButton.icon(
        onPressed: () => _handleFeature('Visit Our Site'),
        icon: const Icon(Icons.language, size: 20),
        label: const Text('Visit yuccaconsult.com',
            style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
        style: ElevatedButton.styleFrom(
          backgroundColor: colorBtnGreen,
          foregroundColor: Colors.white,
