...
class _HomePageState extends State<HomePage> {
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              _goToAuth(); // Then navigate with replacement
            },
            style: ElevatedButton.styleFrom(
  ...
  // ✅ FIXED: Use named route with replacement to avoid back arrow issue
  void _goToAuth() {
    Navigator.pushReplacementNamed(context, '/auth')
        .then((_) {
          // Refresh login status when returning from auth
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF5E9CF),
      // ✅ APPBAR: Company name only, centered, no logo/tagline
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        centerTitle: true,
      ...
      // ✅ BODY: Weather card starts immediately below AppBar
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            // Weather card is FIRST - no branding header above it
            _buildWeatherCard(),
            const SizedBox(height: 12),
            _buildFeatureGrid(),
            const SizedBox(height: 25),
  ...
  Widget _buildFeatureGrid() {
    final features = [
      ...
      {'icon': Icons.lightbulb_outline, 'title': 'Tips',          'color': const Color(0xFFE8A838)},
      {'icon': Icons.bug_report,        'title': 'Disease Info',  'color': const Color(0xFFD95B5B)},
      {'icon': Icons.landscape,         'title': 'Soil Scanner',  'color': const Color(0xFF6B8E3E)},
      {'icon': Icons.shopping_bag,      'title': 'Marketplace',   'color': const Color(0xFF3498DB)},
